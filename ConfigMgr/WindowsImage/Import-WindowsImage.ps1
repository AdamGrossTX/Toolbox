...
param(
    ...
    [Parameter(Mandatory=$True,HelpMessage="Operating System version to service. Default is 1903.")]
    [ValidateSet('1709','1803','1809','1903','1909')]
    [string]
    $OSVersion,

    [Parameter(Mandtory=$True,HelpMessage="Architecture version to service. Default is x64.")]
    [ValidateSet ('x64','x86','ARM64')]
    [string]
    $OSArch,

    [Parameter(Mandatory=$True,HelpMessage="Year-Month of updates to apply (Format YYYY-MM).")]
...
Try {
    ...
    $CompletedMediaPath = Join-Path -Path (Join-Path -Path $SourceMediaRootPath -ChildPath $OSVersion) -ChildPath $Month
